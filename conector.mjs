...
const AFFID = process.env.CAREERJET_AFFID || '';   // gratis en careerjet.com/partners/api
const JOOBLE = process.env.JOOBLE_KEY || '';       // gratis en jooble.org/api/about

const FUENTES = [
  // Vía rápida al volumen real: agregadores que YA licenciaron los portales.
  ...(AFFID ? REGIONES.map(r => ({ tipo: 'careerjet', affid: AFFID, region: r, paginas: 8, empresa: 'Careerjet' })) : []),
  ...(JOOBLE ? REGIONES.map(r => ({ tipo: 'jooble', apikey: JOOBLE, region: r, paginas: 5, empresa: 'Jooble' })) : []),
...
const ADAPTERS = {
  /* ─────────────────────────────────────────────────────────
     CAREERJET — API pública de partner. https://www.careerjet.com/partners/api
     Careerjet YA tiene los acuerdos con los portales (incluido Computrabajo).
     Tú no copias a nadie: consumes un agregador que licenció el inventario.
     Pides un affid gratis en su web. Redirección vía su tracking = así se
     financian, y es lo que hace legal el arreglo.
     ───────────────────────────────────────────────────────── */
  async careerjet(f) {
    const out = [];
    for (let page = 1; page <= (f.paginas || 5); page++) {
      ...
      for (const j of d.jobs) out.push({
        src: 'careerjet', ext: j.url.split('/').pop() || `${j.title}|${j.company}`,
        title: j.title, company: j.company || 'No informada',
        location: j.locations || f.region,
        description: strip(j.description),
        smin: j.salary_min ? Math.round(+j.salary_min) : null,
        smax: j.salary_max ? Math.round(+j.salary_max) : null,
        pub: new Date(j.date).toISOString(),
        apply: j.url,                 // redirección de Careerjet: NO la cambies
  ...
  async jooble(f) {
    const out = [];
    for (let page = 1; page <= (f.paginas || 5); page++) {
      const r = await fetch(`https://cl.jooble.org/api/${f.apikey}`, {
        method: 'POST',
...
if (!FUENTES.length) {
  console.log('\n  Sin fuentes configuradas. Para tener datos REALES ahora mismo:\n' +
              '    1. Pide un affid gratis en https://www.careerjet.com/partners/api\n' +
              '    2. CAREERJET_AFFID=tu_affid node conector.mjs\n' +
              '  Esto barre las 16 regiones y trae avisos reales de los portales que\n' +
              '  Careerjet ya licenció. El HTML seguirá con los 25 de ejemplo hasta entonces.\n');
}
